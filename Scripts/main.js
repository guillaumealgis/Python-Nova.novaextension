...
function alertDeprecatedServerIfNeeded(serverBinPath) {
    const ignoreConfigName =
        nova.extension.identifier + '.alerted-deprecated-server';
    ...
    promise.then(
        (reply) => {
            if (reply.actionIdx == 1) {
                nova.openURL(
                    'https://github.com/mmshivesh/Python-Nova.novaextension/pull/18'
                );
    ...
    return {
        settings: {
            [pylsNamespace]: {
                ...
                rope: {
                    extensionModules: getPreference(
                        'pyls.rope.extensionModules'
                    ),
                    ropeFolder: parseSpaceSeparated(
                        getPreference('pyls.rope.ropeFolder')
                    )
                },
                plugins: {
                    jedi: {
                        enabled: getPreference('pyls.plugins.jedi.enabled'),
                        extra_paths: parseSpaceSeparated(
                            getPreference('pyls.plugins.jedi.extra_paths')
                        ),
                        env_vars: parseJSON(
                            getPreference('pyls.plugins.jedi.env_vars')
                        ),
                        environment: (function () {
                            if (
                                ...
                                console.log(
                                    'Workspace Jedi environment undefined. Using global value'
                                );
                                ...
                                console.log(
                                    'Jedi environment overriden at Workspace. Using workspace value instead.'
                                );
                                return getPreference(
                                    'pyls.plugins.jedi.workspace.environment',
                                    undefined,
                    ...
                    jedi_completion: {
                        ...
                        include_params: getPreference(
                            'pyls.plugins.jedi_completion.include_params'
                        ),
                        include_class_objects: getPreference(
                    ...
                    jedi_definition: {
                        ...
                        follow_imports: getPreference(
                            'pyls.plugins.jedi_definition.follow_imports'
                        ),
                        follow_builtin_imports: getPreference(
                    ...
                    pycodestyle: {
                        ...
                        exclude: parseSpaceSeparated(
                            getPreference('pyls.plugins.pycodestyle.exclude')
                        ),
                        filename: parseSpaceSeparated(
                            getPreference('pyls.plugins.pycodestyle.filename')
                        ),
                        select: parseSpaceSeparated(
                            getPreference('pyls.plugins.pycodestyle.select')
                        ),
                        ignore: conditionalAppendArgumentsToArray(
                            ...
                            parseSpaceSeparated(
                                getPreference('pyls.plugins.pycodestyle.ignore')
                            ),
                            'E501'
                        ),
                        hangClosing: getPreference(
                            'pyls.plugins.pycodestyle.hangClosing'
                        ),
                        maxLineLength: getPreference(
                    ...
                    pydocstyle: {
                        ...
                        convention: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.convention')
                        ),
                        addIgnore: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.addIgnore')
                        ),
                        addSelect: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.addSelect')
                        ),
                        ignore: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.ignore')
                        ),
                        select: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.select')
                        ),
                        match: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.match')
                        ),
                        matchDir: parseSpaceSeparated(
                            getPreference('pyls.plugins.pydocstyle.matchDir')
                        )
                    },
                    pylint: {
                        enabled: getPreference('pyls.plugins.pylint.enabled'),
                        args: parseSpaceSeparated(
                            getPreference('pyls.plugins.pylint.args')
                        ),
                        executable: getPreference(
...
class PythonLanguageServer {
    ...
    addPreferenceObservers() {
        ...
        let workspaceKeys = ['pyls.plugins.jedi.workspace.environment'];
    ...
    async start(path) {
        ...
        if (getPreference('pyls.enableLogging', false)) {
            serverOptions['args'] = [
                '-vv',
                '--log-file',
                getPreference('pyls.logPath', '/tmp/pyls.log')
            ];
        }
